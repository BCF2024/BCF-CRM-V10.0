...
function currentFormData(){
  ...
  if(existing){
    ["driveFolderId","driveFolderUrl","googleFormSubmissionId","googleFormImportedAt","googleFormCreatedAt","googleFormApplication","jotformSubmissionId","jotformImportedAt","jotformCreatedAt","jotformApplication","emailActivity"].forEach(key=>{
...
function bcfApplicationUrl(){
  return String(getCloudSettings().googleFormUrl||DEFAULT_FORM_URL||"").trim();
}
...
$("createGoogleFormBtn").onclick=async()=>{
  if(!cloudConfigured())return alert("Connect the Google Apps Script first using Settings.");
  const btn=$("createGoogleFormBtn");
  try{
    btn.disabled=true;btn.textContent="Creating Google Form...";
    const out=await cloudCall("setupGoogleForm",{});
    $("googleFormUrl").value=out.formUrl||"";$("googleFormSheetId").value=out.spreadsheetId||"";
    setAdminSettings(readAdminSettings());
    alert("BearCrest Google Form created. The form and response spreadsheet are now connected.");
    if(out.editUrl)window.open(out.editUrl,"_blank");
  }catch(error){alert(error.message);}finally{btn.disabled=false;btn.textContent="Create BearCrest Google Form";}
};
$("openGoogleFormBtn").onclick=()=>{const url=$("googleFormUrl").value.trim();if(!url)return alert("Create or paste the Google Form link first.");window.open(url,"_blank");};

$("syncApplicationsBtn").onclick=async()=>{
  if(!cloudConfigured()){alert("Connect the Google Apps Script first using Settings.");return;}
  const button=$("syncApplicationsBtn");
  try{
    button.disabled=true;button.textContent="Checking Google Forms...";
    const settings=getCloudSettings();
    if(!settings.googleFormSheetId)throw new Error("Open Settings and create or connect the Google Form response spreadsheet first.");
    const out=await cloudCall("syncGoogleForms",{spreadsheetId:settings.googleFormSheetId});
    const importedIds=new Set(loans.map(loan=>String(loan.googleFormSubmissionId||loan.jotformSubmissionId||"")).filter(Boolean));
    let added=0,skipped=0,errors=0;
    for(const submission of out.submissions||[]){
      try{
        const submissionId=String(submission.id||"");
        if(!submissionId){errors++;continue;}
        if(importedIds.has(submissionId)){skipped++;continue;}
        const answers=submission.answers||{};
        const loan=normalizeLoan({
          googleFormSubmissionId:submissionId,
          googleFormImportedAt:new Date().toISOString(),
          loanNumber:nextLoanNumber(),
          dateReceived:(submission.created_at||today()).slice(0,10),
          borrowerName:answerValue(answers,["full name","borrower name","applicant name"]),
          phone:answerValue(answers,["phone"]),email:answerValue(answers,["email"]),
          entityName:answerValue(answers,["entity name","borrowing entity","llc"]),
          ...
          rehabBudget:answerValue(answers,["rehab budget","rehab amount","renovation budget","repair budget"]).replace(/[^0-9.]/g,""),
          asIsValue:answerValue(answers,["as-is value","as is value","current value","present value"]).replace(/[^0-9.]/g,""),
          arv:answerValue(answers,["estimated arv","after repair value"]).replace(/[^0-9.]/g,""),
          ...
          propertyType:answerValue(answers,["property type"])||"Single Family",rural:answerValue(answers,["rural"])||"No",
          targetClosing:answerValue(answers,["target closing"]),exitStrategy:answerValue(answers,["exit strategy"]),status:"New Lead",
          notes:`Imported from Google Forms response ${submissionId}`,
          googleFormCreatedAt:submission.created_at||"",
          googleFormApplication:normalizeGoogleFormApplication(submission)
        });
        loans.unshift(loan);importedIds.add(submissionId);added++;
      }catch(error){console.error("Google Form import error",error);errors++;}
    }
    save();saveSyncMeta({lastSync:new Date().toISOString(),lastAdded:added,lastSkipped:skipped,lastErrors:errors});
    alert(`${added} new application${added===1?"":"s"} imported.\n${skipped} already in CRM and skipped.\n${errors} error${errors===1?"":"s"}.`);
  }catch(error){alert(error.message);}finally{button.disabled=false;button.textContent="Sync Google Forms";}
};
...
function normalizeLender(x={},index=0){
  return {
    ...
    guidelineUrl:String(x.guidelineUrl||""),
    profileStatus:String(x.profileStatus||"Needs Research"),
    lastVerified:String(x.lastVerified||""),
    programs:String(x.programs||""),
    minFico:String(x.minFico||""),
    firstTime:String(x.firstTime||""),
    minLoan:String(x.minLoan||""),
    maxLoan:String(x.maxLoan||""),
    maxLtv:String(x.maxLtv||""),
    maxLtc:String(x.maxLtc||""),
    maxLtarv:String(x.maxLtarv||""),
    foreignNational:String(x.foreignNational||""),
    rural:String(x.rural||""),
    states:String(x.states||""),
    why:String(x.why||""),
...
function getLocalLenders(){
  try{
    ...
    if(Array.isArray(stored)&&stored.length)return stored.map(normalizeLender).filter(l=>l.name.toLowerCase()!=="unitas funding");
  }catch(e){}
...
async function loadLenders(showMessage=false){
  try{
    if(cloudConfigured()){
      ...
      lenders=(out.lenders||[]).map(normalizeLender).filter(l=>l.name.toLowerCase()!=="unitas funding");
      localStorage.setItem(LENDER_STORAGE_KEY,JSON.stringify(lenders));
  ...
  list.innerHTML=lenders.map((l,index)=>{
    ...
    return `<div class="lender-row ${l.active===false?"inactive":""}">
      <div class="lender-main">
        ...
        <div class="lender-verified">${l.lastVerified?`Last verified ${esc(l.lastVerified)}`:"Not yet verified"}</div>
      </div>
...
function openLenderEditor(lender=null){
  ...
  $("lenderGuidelineUrl").value=lender?.guidelineUrl||"";
  $("lenderProfileStatus").value=lender?.profileStatus||"Needs Research";
  $("lenderLastVerified").value=lender?.lastVerified||"";
  $("lenderPrograms").value=lender?.programs||"";
  $("lenderMinFico").value=lender?.minFico||"";
  $("lenderFirstTime").value=lender?.firstTime||"";
  $("lenderMinLoan").value=lender?.minLoan||"";
  $("lenderMaxLoan").value=lender?.maxLoan||"";
  $("lenderMaxLtv").value=lender?.maxLtv||"";
  $("lenderMaxLtc").value=lender?.maxLtc||"";
  $("lenderMaxLtarv").value=lender?.maxLtarv||"";
  $("lenderForeignNational").value=lender?.foreignNational||"";
  $("lenderRural").value=lender?.rural||"";
  $("lenderStates").value=lender?.states||"";
  $("lenderWhy").value=lender?.why||"";
...
if($("saveLenderBtn"))$("saveLenderBtn").onclick=async()=>{
  ...
  const payload={
    ...
    guidelineUrl:$("lenderGuidelineUrl").value.trim(),
    profileStatus:$("lenderProfileStatus").value,
    lastVerified:$("lenderLastVerified").value,
    programs:$("lenderPrograms").value.trim(),
    minFico:$("lenderMinFico").value.trim(),
    firstTime:$("lenderFirstTime").value,
    minLoan:$("lenderMinLoan").value.trim(),
    maxLoan:$("lenderMaxLoan").value.trim(),
    maxLtv:$("lenderMaxLtv").value.trim(),
    maxLtc:$("lenderMaxLtc").value.trim(),
    maxLtarv:$("lenderMaxLtarv").value.trim(),
    foreignNational:$("lenderForeignNational").value,
    rural:$("lenderRural").value,
    states:$("lenderStates").value.trim(),
    why:$("lenderWhy").value.trim(),
...
function v5RenderRecentLoans(){
  ...
  box.innerHTML=recent.length?recent.map(l=>`
    ...
    </button>`).join(""):'<div class="empty-state">No loans yet. Add your first loan or sync Google Forms.</div>';
}
...
function applicationRowsHtml(loan){
  ...
  const jotRows=(loan?.googleFormApplication||loan?.jotformApplication||[]).map(x=>[x.question,x.answer]);
  const jotSection=jotRows.length?`<h2 class="section-title">Additional Google Form Answers</h2>${renderApplicationRows(jotRows,false)}`:"";
...
function bcfSuggestedArv(data,selectedIndexes,input={}){
  ...
  if(averageSalePrice)methods.push({name:'Average Selected Sale',value:averageSalePrice});
  if(medianSalePrice)methods.push({name:'Median Selected Sale',value:medianSalePrice});
...
function recalculateDealArv(){
  ...
  if(selected.length<3){
    if(status)status.textContent=`Keep at least 3 comparable sales selected. Current total: ${selected.length}.`;
    return;
  }
  const data=lastDealAnalysis.data;
  const input={...lastDealAnalysis.input};
  renderDealAnalysis(data,input,selected);
  const refreshed=document.getElementById('recalculateDealArvStatus');
  if(refreshed)refreshed.textContent=`ARV updated automatically using ${selected.length} selected comps.`;
}
window.recalculateDealArv=recalculateDealArv;
document.addEventListener('change',event=>{
  if(!event.target.matches('.deal-comp-select'))return;
  recalculateDealArv();
});
function openCompSearch(site,address){
  const query=String(address||'').trim();
  if(!query)return;
  const domain=site==='zillow'?'zillow.com':site==='redfin'?'redfin.com':'realtor.com';
  window.open(`https://www.google.com/search?q=${encodeURIComponent('site:'+domain+' '+query)}`,'_blank','noopener');
}
window.openCompSearch=openCompSearch;
...
function renderDealAnalysis(data,input,selectedIndexes){
  ...
  const warnings=[];
  if(calc.count<3)warnings.push('ARV is not calculated until at least three comparable sales remain selected.');
  ...
  $("dealAnalyzerResults").innerHTML=`<div class="deal-summary-grid">
    ...
    <div class="deal-metric"><small>Selected Comparable Sales</small><strong>${calc.count}</strong></div>
  ...
  </div>${calc.methods?.length?`<div class="deal-comps"><h3>BearCrest ARV Methods</h3><table><thead><tr><th>Valuation Method</th><th>Indicated ARV</th></tr></thead><tbody>${calc.methods.map(m=>`<tr><td>${esc(m.name)}</td><td><b>${currency0(Math.round(m.value/1000)*1000)}</b></td></tr>`).join("")}</tbody></table><p style="margin:10px 0 0"><b>Recommended ARV:</b> ${currency0(automatedArv)} — reconciled from the middle of the supported methods to reduce the impact of one unusually high or low result.</p></div>`:''}<div class="deal-note"><b>How to use this:</b> All RentCast comps start selected. Uncheck sales that are poor matches, distressed, unrenovated, or otherwise unsuitable. The ARV updates automatically. RentCast does not certify renovation condition, so use the Zillow, Redfin, and Realtor buttons beside a comp to review available listing photos and details.${manualArv?'<br><b>Manual override active:</b> The entered override is being used.':''}${warnings.length?'<br><b>Review:</b> '+warnings.map(esc).join(' '):''}</div>
  <div class="deal-actions"><span id="recalculateDealArvStatus">All comps selected. Uncheck any comp you do not want used; ARV updates automatically.</span></div>
...
function loanApplicationAnswer(loan,needles){
  const rows=Array.isArray(loan?.googleFormApplication)?loan.googleFormApplication:(Array.isArray(loan?.jotformApplication)?loan.jotformApplication:[]);
...
async function matchCurrentLoan(){
  ...
  if(Array.isArray(saved.googleFormApplication))loan.googleFormApplication=saved.googleFormApplication; else if(Array.isArray(saved.jotformApplication))loan.jotformApplication=saved.jotformApplication;
...
// ===== BearCrest Version 11.1: Paste-safe property addresses =====
function bcfCleanAddressText(value){
  let text=String(value||'').replace(/\r?\n+/g,', ').replace(/\s+/g,' ').replace(/,+/g,',').trim();
  const match=text.match(/\b\d{1,6}\s+[A-Za-z0-9.'#\- ]+(?:Street|St|Road|Rd|Avenue|Ave|Boulevard|Blvd|Drive|Dr|Lane|Ln|Court|Ct|Way|Parkway|Pkwy|Highway|Hwy|Place|Pl|Terrace|Ter|Circle|Cir)\b[^;|]*/i);
  return (match?match[0]:text).replace(/\s+,/g,',').trim();
}
['propertyAddress','dealAddress','matchAddress','contactAddress'].forEach(id=>{
  const el=$(id);if(!el)return;
  el.addEventListener('paste',()=>setTimeout(()=>{el.value=bcfCleanAddressText(el.value);el.dispatchEvent(new Event('input',{bubbles:true}));},0));
